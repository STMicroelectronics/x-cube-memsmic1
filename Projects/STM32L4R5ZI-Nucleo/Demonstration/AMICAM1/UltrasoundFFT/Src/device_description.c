...
extern uint8_t amicam1_com_id;
extern SM_Init_Param_t AMICAM1_Init_Param;
#define WRITE_BUFFER_SIZE_AMICAM1      (uint32_t)(65000)
...
void set_default_description(void)
{
  ...
  strcpy(tempDeviceDescriptor->alias, "NUCLEOL476RG_001");

  /**********/

  /* AMICAM1 */
  amicam1_com_id = COM_AddSensor();

  tempSensor = COM_GetSensor(amicam1_com_id);

  /* SENSOR DESCRIPTOR */
  strcpy(tempSensor->sensorDescriptor.name, "AMICAM1");
  ...
  tempSensor->sensorStatus.sdWriteBufferSize = WRITE_BUFFER_SIZE_AMICAM1;
  ...
  AMICAM1_Init_Param.ODR = tempSensor->sensorStatus.ODR;
  AMICAM1_Init_Param.FS[0] = tempSensor->sensorStatus.subSensorStatus[0].FS;
  AMICAM1_Init_Param.subSensorActive[0] = tempSensor->sensorStatus.subSensorStatus[0].isActive;
}
