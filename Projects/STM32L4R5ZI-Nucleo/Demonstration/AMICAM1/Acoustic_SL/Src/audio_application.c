...
{
  if (AcousticSL_Data_Input((int16_t *)&PCM_Buffer[0], (int16_t *)&PCM_Buffer[1],
                            NULL, NULL, &libSoundSourceLoc_Handler_Instance) == 1U)
  {
...
void Audio_Libraries_Init(void)
{
  ...
  libSoundSourceLoc_Handler_Instance.M12_distance = ONBOARD_MICS;
...
void SW_Task2_Callback(void)
{
  ...
  if (result[0] != -1)
  {
    ...
    (void)HAL_UART_Transmit(&hlpuart1, (uint8_t *)"    ", 4, 0xFFFF);
    (void)HAL_UART_Transmit(&hlpuart1, (uint8_t *)"\r", 1, 0xFFFF);
    (void)HAL_UART_Transmit(&hlpuart1, (uint8_t *)output, (uint16_t)n, 0xFFFF);
  }
}
