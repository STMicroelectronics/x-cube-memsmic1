...
#define WRITE_BUFFER_SIZE_AMIC_ARRAY      (uint32_t)(65000)
...
void set_default_description(void)
{
  ...
  /* AMIC_ARRAY */
  AMic_Array_com_id = COM_AddSensor();

  tempSensor = COM_GetSensor(AMic_Array_com_id);

  /* SENSOR DESCRIPTOR */
  strcpy(tempSensor->sensorDescriptor.name, "AMIC_ARRAY");
  ...
  tempSensor->sensorStatus.sdWriteBufferSize = WRITE_BUFFER_SIZE_AMIC_ARRAY;
  ...
  AMIC_ARRAY_Init_Param.ODR = tempSensor->sensorStatus.ODR;
  AMIC_ARRAY_Init_Param.FS[0] = tempSensor->sensorStatus.subSensorStatus[0].FS;
  AMIC_ARRAY_Init_Param.subSensorActive[0] = tempSensor->sensorStatus.subSensorStatus[0].isActive;

  maxWriteTimeSensor[AMic_Array_com_id] = 1000 * WRITE_BUFFER_SIZE_AMIC_ARRAY / (uint32_t)(AMIC_ARRAY_Init_Param.ODR * 2);
